...
test.use({storageState:'datanew/Leaftaps_Login.json'})

test.describe.parallel("Test Execution in parallel mode",async()=>{
...
test.beforeAll("Read the value from CSV file",async()=>{
    ...
    records=parse(fs.readFileSync("datanew/LT.csv"),{columns:true})
